...
export default function App() {
  ...
  useEffect(() => {
    ...
    players.forEach(({ plate, position }) => {
      ...
      image.src = '/car.png'
      image.onload = () => getCanvas().drawImage(image, position.x, position.y, position.height, position.width)
    })
        ...
        <div className="header">
          <h1>Você é <b>{me?.plate}</b></h1>
          {
            ...
            && <h2 style={{whiteSpace: 'nowrap'}}>
              Sua placa é
              do <b
              style={{textTransform: 'uppercase'}}>{plateLabel}</b> e {!['alagoas', 'bahia', 'sergipe'].includes(plateLabel.toLowerCase()) && ' não '} faz
              parte do <b>NORDESTE 3</b>
            </h2>
          }
          <h3 style={{fontSize: '1.4rem', paddingTop: '10px'}}>{msToTime(time * 1000)}</h3>
        </div>
      )}
          ...
          <Accordion.Item className="AccordionItem" value={`item-${index}`} key={`item-${index}`}>
            <Accordion.Header className="AccordionHeader">
              <Accordion.Trigger className="AccordionTrigger">
                <p><b>{new Intl.NumberFormat('pt-BR', {
                  style: 'currency',
                  currency: 'BRL'
                }).format(ticket.total / 100)}</b></p>
